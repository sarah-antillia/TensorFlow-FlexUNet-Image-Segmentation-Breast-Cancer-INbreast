...
#
# 2025/07/30 ImageMaskDatasetGenerator.py
...
class ImageMaskDatasetGenerator:
  ...
  def generate(self):
    masks_files = glob.glob(self.masks_dir + "/*.png")
    ...
    for i in range(l1):
      self.generate_mask_files (masks_files[i],  i, self.output_masks_dir) 
      self.generate_image_files(image_files[i],  i, self.output_images_dir) 
  ...
  def generate_mask_files(self, mask_file, index, output_dir):   
  ...
  def generate_image_files(self, image_file, index, output_dir):
...
if __name__ == "__main__":
  try:
    images_dir        = "./INbreast/INBREAST-SELECTED-IMGS/"
    masks_dir         = "./INbreast/INBREAST-SELECTED-MSKS/"
    output_dir        = "./INbreast-master/"
    output_images_dir = "./INbreast-master/images/"
    output_masks_dir  = "./INbreast-master/masks/"
